...
import s1.receiving.ReceivingGrpc;
import s1.receiving.ReceivingGrpc.ReceivingBlockingStub;
import s2.shipping.ShippingGrpc;
import s3.inventory.InventoryGrpc;
...
public class ControllerGUI implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (label.equals("Invoke Service 1")) {
			...
			ReceivingBlockingStub blockingStub = ReceivingGrpc.newBlockingStub(channel);

			//preparing message to send
			//s1.receiving.RequestMessage request = s1.receiving.RequestMessage.newBuilder().setText(entry1.getText()).build();
